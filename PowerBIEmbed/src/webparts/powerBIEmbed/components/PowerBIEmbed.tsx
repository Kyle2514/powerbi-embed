...
const PowerBIEmbed: React.FC<IPowerBiEmbedProps> = ({
  userEmail,
  reportId,
  datasetId,
  hasRLS,
  ...
  React.useEffect(() => {
    if (
      ...
      setError(
        "report ID, dataset ID, report workspace Id, and dataset workspace Id are required to embed the report."
      );
      return;
    }
    ...
    const fetchAndEmbedReport = async (): Promise<void> => {
      try {
        const response = await fetch(
          `https://powerbiembedappservice-fubkgqhrhrdsdtdn.francecentral-01.azurewebsites.net/api/embed-info?userEmail=${encodeURIComponent(
            userEmail
          )}&reportId=${encodeURIComponent(
            reportId
          )}&datasetId=${encodeURIComponent(datasetId)}&hasRLS=${
            hasRLS ? "yes" : "no"
          }&reportWorkspaceId=${encodeURIComponent(
            reportWorkspaceId
          )}&datasetWorkspaceId=${encodeURIComponent(datasetWorkspaceId)}`
          /* for testing 
          `http://localhost:3000/api/embed-info?userEmail=${encodeURIComponent(
            userEmail
          )}&reportId=${encodeURIComponent(
            reportId
          )}&datasetId=${encodeURIComponent(datasetId)}&hasRLS=${
        ...
        // Log the embed token for debugging
        console.log("Received embed token:", embedInfo.embedToken);
        ...
        if (reportRef.current) {
          ...
          report.on(
            "error",
            (event: service.ICustomEvent<IPowerBIErrorDetail>) => {
        ...
        console.error(
          "Error fetching embed info or embedding report:",
          errorMessage
        );
  ...
  return (
    <div>
      ...
      <div ref={reportRef} style={{ height: "800px", width: "800px" }} />
    </div>
  );
};
